...
<head>
  ...
  if(isset($_POST['submitfee'])){
    ...
    $fee=0;
      ...
      $data=mysqli_query($con,"INSERT INTO student_fee (std_id,desp,mof,yof,fee,arrears,paid,due,dates)VALUES ('$std_id','$desp','$mof','$yof','$fee','$newarr','$paying','1','$dates')")or die( mysqli_error($con) );

      if(empty($_GET['refer'])){
      header("location:submitfee.php?done=1");

    }else{
      header("location:viewstudent.php?viewid=$std_id");
  ...
  if(isset($_POST['addallarr'])){
    $msg="Unsuccessful" ;
      ...
      $rowsx =mysqli_query($con,"SELECT * FROM student_fee WHERE std_id=$id AND mof=$mof  AND yof=$yof" ) or die(mysqli_error($con));
      $n=0;
     ...
     if(empty($due) OR empty($pay)){
      ...
      $data=mysqli_query($con,"INSERT INTO student_fee (std_id,mof,yof,arrears,desp,fine,paid,fee,due,dates)VALUES ('$id','$mof','$yof','$newarr','$desp','$newarr','0','$fee','1','$dates')")or die( mysqli_error($con) );
...
<?php } if(empty($name)) $msg='Not Found'; } ?>
  ...
  <section id="configuration">
    <div class="row">
      <div class="col-12">
        <div class="card">
          <div class="card-header">
            <h4 class="card-title">Unpaid Students Fee</h4>
            <a class="heading-elements-toggle"><i class="la la-ellipsis-v font-medium-3"></i></a>
            <div class="heading-elements">
              <ul class="list-inline mb-0">
                <li><a data-action="collapse"><i class="ft-minus"></i></a></li>
  ...
  <table id="stdtable" class="table table-striped table-bordered zero-configuration">
    ...
    <tbody>
      ...
      $rowsx =mysqli_query($con,"SELECT due FROM student_fee WHERE std_id=$id AND mof=$mof  AND yof=$yof " ) or die(mysqli_error($con));
      $n=0;
     ...
     if(empty($pay)){
      ...
      $rowsxx =mysqli_query($con,"SELECT * FROM student_fee WHERE std_id=$id ORDER BY id desc LIMIT 1" ) or die(mysqli_error($con));
      $n=0;
...
<br><br>
<h4>Add All Unpaid Students Fee to Arrear </h4>
<br><br>
   <table class="table table-bordered">
